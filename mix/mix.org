...
#+end_src
#+begin_export latex
We call our user jdoe. Its password is ``password'', which we enter in
hashed form. The default shell of jdoe is the bash and the account
...
#+end_src
#+begin_export latex
To help users, we copy one file, \ty{README}, into the user's home
directory.
#+end_export
...
#+end_src
#+begin_export latex
In the \ty{README} we explain how to install a package and how to
install a private executable.
#+end_export
#+begin_src sh <<README>>=
...
#+end_src
#+begin_export latex
We set the user and its working directory. Then we set the home
...
#+end_src
#+begin_export latex
Inside \ty{start.sh} we run \ty{docker} interactively, set \ty{mix} as
its host name, and make sure we can use \ty{ctrl-p} to reach the
previous command.
#+end_export
#+begin_src sh <<start>>=
